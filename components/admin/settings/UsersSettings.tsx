...
import { isActive, useAdminUsers, useCurrentUser, useUserActions, type NewUserInput } from "@/components/admin/providers/CurrentUserProvider";
...
function AddUserDialog({ onClose }: { onClose: () => void }) {
  ...
  // New accounts start at the least privilege (CLAUDE.md Section 7).
  const [draft, setDraft] = useState<Draft>({ role: "viewer", department: currentUser.department, languages: ["English"], location: LOCATIONS[0], active: true });

  const [saving, setSaving] = useState(false);

  async function save() {
    const input: NewUserInput = { name, email, role: draft.role, department: draft.department, languages: draft.languages, location: draft.location };
    setSaving(true);
    const result = await addUser(input, currentUser);
    setSaving(false);
    if (!result.ok) return toast.error("Not added", { description: result.reasons[0] });
  ...
  const footer = (
    <>
      ...
      <button type="button" onClick={() => void save()} disabled={saving || !name.trim() || !email.trim()} className={buttonClasses.primary}>
        Add account
      </button>
    </>
  );
  ...
  return (
    <DialogShell titleId={`${formId}-title`} title="Add an account" onClose={onClose} footer={footer} size="medium">
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <RoleFields draft={draft} setDraft={setDraft} lockRole={false} formId={formId} />
        <p className={hintClasses}>New accounts start as Viewers. Give people the least access their work needs.</p>
      </div>
    </DialogShell>
  );
}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block truncate text-xs text-slate-500">
                        {user.email ?? "Preview persona"} · {user.location} · {user.languages.join(" & ")}
                      </span>
                    </span>
                  </div>
                </td>
