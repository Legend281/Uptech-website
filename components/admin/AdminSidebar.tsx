...
type NavItem = {
  ...
  icon: string;
  href?: string;
  /** Not built yet this pass — renders as a muted, non-interactive row with a "Soon" pill instead of a dead link. */
  soon?: boolean;
  adminOnly?: boolean;
  ...
  if (item.soon || !item.href) {
    return (
      <div className={`${rowClasses} cursor-default text-slate-500`}>
        ...
        <span className="rounded-full bg-white/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-500">
          Soon
        </span>
      </div>
    );
  ...
  const content = (
    <div className="flex h-full flex-col bg-navy-950">
      <div className="flex items-center gap-2 border-b border-white/10 px-5 py-5">
        <Image src="/UPTECH_LOG.png" alt="Uptech Consulting & Outsourcing" width={572} height={233} className="h-9 w-auto" />
        <span className="rounded-md bg-white/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-teal-400">
          Admin
        </span>
      </div>

      <nav data-lenis-prevent className="min-h-0 flex-1 space-y-6 overflow-y-auto px-3 py-6">
        {navGroups.map((group) => {
      ...
      <div className="px-4 pb-4">
        <a
          ...
          className="group block overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-navy-900 to-navy-950 p-4"
        >
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-teal-400 to-blue-accent text-xs font-bold text-white">
        ...
        <label className="mt-3 block">
          ...
          <select
            ...
            className="w-full rounded-lg border border-white/10 bg-navy-900 px-2.5 py-2 text-xs font-medium text-slate-200 focus:border-teal-400 focus:outline-none focus:ring-1 focus:ring-teal-400/40"
          >
        ...
        <button
          ...
          className="mt-3 flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-semibold text-slate-400 hover:bg-white/5 hover:text-white"
        >
