...
import { useNotificationPrefs, useServicePages, useSettings } from "@/components/admin/providers/SettingsProvider";
...
export function ReviewCycleSettings() {
  const currentUser = useCurrentUser();
  const { settings, saveReviewCycles } = useSettings();
  const pages = useServicePages();
  const [draft, setDraft] = useState<Record<string, ReviewCycle>>(settings.reviewCycles);
  const dirty = JSON.stringify(draft) !== JSON.stringify(settings.reviewCycles);
  ...
  return (
    <section className={`${CARD_SURFACE} p-5`}>
      ...
      <div className="space-y-3">
        {pages.map((page) => {
          const cycle = draft[page.id] ?? { cadenceDays: page.reviewCadenceDays, dueSoonDays: 30 };
      ...
      {editable && (
        <SaveBar
          ...
          onDiscard={() => setDraft(settings.reviewCycles)}
          onSave={() => {
            void toastResult(saveReviewCycles(draft, currentUser), "Review cycles saved");
          }}
        />
      )}
    </section>
