 ...
 * calendar string — so the staleness demo and "3 hours ago" activity feed
 * stay meaningful whenever this is actually reviewed, not just today.
 */
...
/** career-marketing is the only service that belongs to Career Services Operations — every other value (both formalisation jurisdictions, tax, CNPS, and the "not sure"/"other" catch-alls) is Business Formalisation & Compliance's. */
 ...
 * 2026 date) purely so the staleness panel below has genuine on-track /
 * due-soon / overdue examples to show.
 */
...
/*
 * Dashboard-preview lead data only — powers the stat cards, the pipeline
 * stage counts, and a short "Recent Leads" list. The full Leads module
 * (filtering, status changes, notes, a detail page) is a separate, later
 * build; there's no /admin/leads route yet, so nothing here links to one.
 */
